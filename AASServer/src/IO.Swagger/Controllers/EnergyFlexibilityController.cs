        ...
        public virtual IActionResult runDayAheadFlexibilityPost([FromBody] FlexibilityDayAheadRequest body, [FromHeader] String Authorization)
        {
            ...
            foreach (JToken load in loads)
            {
                ...
                if (dischargeWh > 0)
                    ...
                    {
                        ChessId = loadId,
                        Entry = new IoT.Services.ChessStatus { status = "ForceDischarge", capacity=dischargeWh.ToString(), service = loadId, starttime = "00:00", endtime = "23:59", recurrence = recurrence, cycleCost = variableCostPerKwh, cycleCostUnit = "currency/kWh" },
                        IsDischarge = true,
                ...
                if (chargeWh > 0)
                    ...
                    {
                        ChessId = loadId,
                        Entry = new IoT.Services.ChessStatus { status = "ForceCharge", capacity=chargeWh.ToString(), service = loadId, starttime = "00:00", endtime = "23:59", recurrence = recurrence, cycleCost = variableCostPerKwh, cycleCostUnit = "currency/kWh" },
                        IsDischarge = false,
                        Remaining = chargeWh,
                        CostPerWh = costPerWh,
                        EligibleStart = eligibleStart,
