...
using huaweiBESSadapter;
using System.Globalization;

namespace huaweiBESSadatper.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        protected void polling(int asset)
        {
            ...
            String json = "{\r\n  \"sn\": \"" + chess.deviceData.deviceSN + "\",\r\n  \"variables\": [\"batteryChargeTotal\", \"batteryDischargeTotal\"]\r\n}";
            String response = Post(chess.location + "/op/v0/device/real/query", json, authToken);

            RealDataResponse realData = (RealDataResponse)JsonConvert.DeserializeObject(response);

            Double totalEnergy = 0;
            foreach (RealData rd in realData.result)
            {
                totalEnergy += rd.value;
            }
            ...
            while (true)
            {
                ...
                try {
                    ...
                    foreach (Status status in chess.status) 
                    {
                        ...
                        response = Post(chess.location + " /op/v0/device/scheduler/enable", JsonConvert.SerializeObject(scheduler), authToken);
                    ...
                    json = "{\r\n  \"sn\": \"" + chess.deviceData.deviceSN + "\",\r\n  \"variables\": [\"batteryChargeTotal\", \"batteryDischargeTotal\", \"temperature\"]\r\n}";
                    response = Post(chess.location + "/op/v0/device/real/query", json, authToken);

                    realData = (RealDataResponse)JsonConvert.DeserializeObject(response);

                    foreach (RealData rd in realData.result)
                    {

                        if (rd.variable.Equals("batteryChargeTotal"))
                            chargeTotal += rd.value;
                        if (rd.variable.Equals("batteryDischargeTotal"))
                            dischargeTotal += rd.value;
                        if (rd.variable.Equals("temperature"))
                            temperature = rd.value;
        ...
        public IActionResult Init([Required][FromBody] CHESS body, [FromHeader] String Authorization)
        {
            ...
            startInfo.Arguments = "deploy.sh " + body.id + " " + body.location + " " + body.topic;
        ...
        public IActionResult Status([FromRoute] String id, [FromHeader] String Authorization)
        {
            ...
            foreach (CHESS chess in assets)
            {
                    ...
                    if (chess.id.Equals(id))
                    {
                        ...
                        String response = Post(chess.location + "/op/v0/device/scheduler/get", json, Authorization);
                        if (response == null)
                            return StatusCode(500);


                        SchedulerResponse currentSchedule = (SchedulerResponse)JsonConvert.DeserializeObject(response);


                        if (currentSchedule != null)
                        {

                            if (currentSchedule.errno == 0)
                                foreach (Schedule schedule in currentSchedule.Result.groups)
                                {
                                    ...
                                    status.service = schedule.workMode;
                                    status.capacity = schedule.fdSoc.ToString();
                                    status.status = schedule.enable.ToString();
            ...
            foreach (CHESS chess in assets)
            {
                    ...
                    if (chess.id.Equals(id))
                    {
                        ...
                        if (chess.deviceData.hasBattery && chess.deviceData.status == 1)
                        {
                            ...
                            // now set up the scehdule for the assets in the CHESS
                            ...
                            scheduler.deviceSN = chess.deviceData.deviceSN;
                            String response = Post(chess.location + " /op/v0/device/scheduler/enable", JsonConvert.SerializeObject(scheduler), Authorization);
